...
def training_config(arc=ARCHITECTURE, device=DEVICE, optimizer=OPTIMIZER,weight_decay=WEIGHT_DECAY,pre_trained=PRE_TRAINED):
    ...
    model = DANNModel(backbone_name=arc,in_channels=3,num_classes=1,device=device,pretrained=True)    
...
def print_tensorboard(writer, FOLD, DATASET, lambda_grl, train_loss, domain_loss, train_f1, val_dom_loss, val_f1, val_auc, snd_score, im_score, test_auc, epoch):
    # Train Metrics
    writer.add_scalar(f"Fold{FOLD}_{DATASET}/Train/Loss", train_loss, epoch)
    writer.add_scalar(f"Fold{FOLD}_{DATASET}/Train/Domain_Loss", domain_loss, epoch)
    writer.add_scalar(f"Fold{FOLD}_{DATASET}/Train/F1", train_f1, epoch)

    # Validation Metrics
    writer.add_scalar(f"Fold{FOLD}_{DATASET}/Val/Domain_Loss", val_dom_loss, epoch)
    writer.add_scalar(f"Fold{FOLD}_{DATASET}/Val/F1", val_f1, epoch) # Removido o f_ para f
    writer.add_scalar(f"Fold{FOLD}_{DATASET}/Val/AUC", val_auc, epoch)
    
    # Domain Adaptation Specific Scores
    writer.add_scalar(f"Fold{FOLD}_{DATASET}/Val/SND_Score", snd_score, epoch)
    ...
    # Hyperparameters
    writer.add_scalar(f"Fold{FOLD}_{DATASET}/Params/LambdaGRL", lambda_grl, epoch)
