...
def test_with_tent(model, loader, criterion, device, TENT_STEPS, TENT_LR, episodic=True):
    ...
    # 5. Cálculo de Métricas (igual ao AdaBN)
...
def train_kfold(dataset, DATASET_MANY, ARCHITECTURE, criterion, optimizer, scheduler,
    ...
    for DATASET_NAME in df['dataset'].unique():
        for FOLD in range(n_splits):
            ...
            # Inicializa arquitetura limpa (Necessário para carregar o checkpoint)
            ...
            test_loader = DataLoader(dataset_test, shuffle=True, **dl_kwargs)
            ...
            test_loss, test_acc, test_sens, test_spec, test_f1, test_auc, best_threshold = test_with_tent(
                model=model, 
                loader=test_loader, 
                criterion=criterion, 
                device=device,
