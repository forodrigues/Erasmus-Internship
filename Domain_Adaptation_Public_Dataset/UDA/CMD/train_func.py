...
def cmd_loss(source, target, k=5):
    """
    Calcula a Central Moment Discrepancy (CMD) entre as features do domínio fonte e alvo.
    k: Número de momentos a alinhar (K=1 é só média, K=2 média+var, etc. Padrão é 5).
    """
    # 1. Alinhamento de Primeira Ordem (Média)
    x1 = source.mean(0)
    x2 = target.mean(0)
    loss = torch.norm(x1 - x2, p=2)  # Distância Euclidiana entre as médias

    # 2. Alinhamento de Momentos Superiores (Central Moments)
    # Centrar os dados
...
# ==============================================================================
def train_one_epoch(model, sup_loader, unsup_loader, criterion, optimizer, scaler, DATASET, device, K):
    ...
    for sup_batch in sup_loader:
        ...
        with torch.cuda.amp.autocast():
            ...
            if imgs_unsup.size(0) > 1:
                ...
                # Garantir que as features são 2D (Batch, Dim)
                ...
                # Ativação sigmoid nas features como é comum em CMD para limitar o domínio
                f_sup_sig = torch.sigmoid(features_sup)
                f_unsup_sig = torch.sigmoid(features_unsup)
                
                loss_cmd = cmd_loss(f_sup_sig, f_unsup_sig, k=K)
            
            # Soma direta das perdas
            loss = loss_sup + loss_cmd 
...
def validate(model, loader, criterion, DATASET, device, test=False, best_threshold=None, unsup_loader=None):
    ...
    # --- Unsupervised Metrics Calculation (SND & IM Score) ---
    snd_score = 0.0
    im_score = 0.0
...
def train_kfold(dataset, DATASET_MANY, ARCHITECTURE, criterion, optimizer_cls, scheduler_cls,
    ...
    logging.info(f"Alpha: {K}")
    log_dir = f"../Tensorboard/CMD_K{K}" 
    ...
    for DATASET_NAME in df['dataset'].unique():
        for FOLD in range(n_splits):
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                tr_metrics = train_one_epoch(model, train_loader, unsup_loader, criterion, optimizer, scaler, DATASET_NAME, device, K)
                ...
                logging.info(
                    f"Epoch {epoch:03d} | "
                    f"Tr Loss: {tr_loss:.4f} | Class: {tr_loss_sup:.4f} | Cmd: {tr_loss_cmd:.6f} | Acc: {tr_acc:.4f} | F1: {tr_f1:.4f} || "
                    f"Val Loss: {val_loss:.4f} | F1: {val_f1:.4f} | AUC: {val_auc:.4f} || SND: {val_snd:.4f} | IM: {im_score:.4f} | F1 TEST: {test_f1:.4f} | AUC TEST: {test_auc:.4f}"
                )


                print_tensorboard(writer, FOLD, DATASET_NAME, tr_loss,tr_loss_cmd, tr_f1,  val_f1, val_auc,val_snd,im_score,test_auc, epoch)
            ...
            if SAVE:
                os.makedirs("checkpoints_CMD", exist_ok=True)
                path = f"checkpoints_CMD/{DATASET_NAME}_fold{FOLD}_best.pth"
