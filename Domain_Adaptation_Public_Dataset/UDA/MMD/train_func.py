...
def calculate_snd(features, temperature=0.2): # Estava a 0.05
...
def mmd_loss(source, target, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    """
    Calcula a Loss MMD.
    Recebe as features já achatadas (flatten) [Batch, Dim].
    """
    ...
    for sup_batch in sup_loader:
        ...
        with torch.cuda.amp.autocast(): # Garante que o bloco está identado no autocast
            ...
            if alpha > 0:
                ...
                # Só calcula MMD se tivermos amostras suficientes
                if imgs_unsup.size(0) > 1:
                    ...
                    loss_mmd_val = mmd_loss(features_sup, features_unsup)
                    
                    loss_mmd = torch.clamp(loss_mmd_val, min=0.0)
...
def train_kfold(dataset, DATASET_MANY, ARCHITECTURE, criterion, optimizer_cls, scheduler_cls,
    ...
    log_dir = f"../Tensorboard/MMD_ALPHA{ALPHA}" 
    ...
    for DATASET_NAME in df['dataset'].unique():
        for FOLD in range(n_splits):
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                logging.info(
                    f"Epoch {epoch:03d} | "
                    f"Tr Loss: {tr_loss:.4f} | Class: {tr_loss_sup:.4f} | Mmd: {tr_loss_mmd:.4f} | Acc: {tr_acc:.4f} | F1: {tr_f1:.4f} || "
                    f"Val Loss: {val_loss:.4f} | F1: {val_f1:.4f} | AUC: {val_auc:.4f} || SND: {val_snd:.4f} | IM: {im_score:.4f} | F1 TEST: {test_f1:.4f} | AUC TEST: {test_auc:.4f}"
                )

                print_tensorboard(writer, FOLD, DATASET_NAME, tr_loss,tr_loss_mmd, tr_f1,  val_f1, val_auc,val_snd,im_score,test_auc, epoch)
            ...
            if SAVE:
                os.makedirs("checkpoints_MMD", exist_ok=True)
                path = f"checkpoints_MMD/{DATASET_NAME}_fold{FOLD}_best.pth"
