...
# ==============================================================================
# NOVA FUNÇÃO DE LOSS: CORAL (Correlation Alignment)
# ==============================================================================
def coral_loss(source, target):
    """
    Calcula a perda CORAL (Correlation Alignment) entre os domínios fonte e alvo.
    Baseado na norma de Frobenius da diferença entre as matrizes de covariância.
    """
    d = source.size(1) 
    ns, nt = source.size(0), target.size(0)

    # Covariância da Fonte
    ...
    # Frobenius norm ao quadrado da diferença
    loss = torch.sum((xc - xct) ** 2)
    loss = loss / (4 * d * d)
    return loss

# ==============================================================================
# FUNÇÃO DE TREINO COM AMP (Mixed Precision) - ADAPTADA PARA CORAL
# ==============================================================================
...
def train_one_epoch(model, sup_loader, unsup_loader, criterion, optimizer, scaler, DATASET, device, alpha):
    ...
    for sup_batch in sup_loader:
        ...
        with torch.cuda.amp.autocast():
            # Retorna logits e features (flattened ou spatial)
            ...
            if alpha > 0:
                ...
                # Para calcular covariância, precisamos que o batch size > 1
                if imgs_unsup.size(0) > 1:
                    _, features_unsup = model(imgs_unsup, return_features=True)
                    
                    # Certificar que as features são 2D (Batch, Dim) para calcular covariância
                    ...
                    # NOTA: Para CORAL, NÃO normalizamos (F.normalize) as features antes.
                    # Queremos alinhar as estatísticas de segunda ordem reais.
                    loss_coral = coral_loss(features_sup, features_unsup)
...
# ==============================================================================
# FUNÇÃO DE VALIDAÇÃO - ADAPTADA PARA CORAL
# ==============================================================================
...
def validate(model, loader, criterion, DATASET, device, test=False, best_threshold=None, unsup_loader=None):
    ...
    with torch.no_grad():
        ...
        for batch in loader:
            ...
            # Achatamos as features para garantir consistência no cálculo global
    ...
    if unsup_loader is not None:
        ...
        if len(target_features_list) > 0 and len(source_features_list) > 0:
            ...
            # 3. CORAL Loss na Validação (Monitorização do Alinhamento)
            # Nota: CORAL pode ser computacionalmente pesado em batches muito grandes (matriz features x features)
            # Mas geralmente é rápido o suficiente para dimensionalidade de CNNs comuns
            val_coral_alignment = coral_loss(all_features_source, all_features_target).item()
    ...
    if test:
        ...
        return avg_loss, acc, sens, spec, f1, auc, best_threshold, snd_score, im_score, val_coral_alignment
...
def train_kfold(dataset, DATASET_MANY, ARCHITECTURE, criterion, optimizer_cls, scheduler_cls,
    ...
    # Ajuste o Alpha conforme necessário. CORAL geralmente usa valores maiores que MMD (e.g., 1, 10, 100)
    # dependendo da magnitude da loss supervisionada vs loss coral.
    ALPHA = 0
    logging.info(f"Alpha: {ALPHA} | Strategy: Pooled Cross-Validation (Domain Adaptation - CORAL)")
    
    log_dir = f"../Tensorboard/coral_unsup2_{ALPHA}" 
    ...
    for DATASET_NAME in df['dataset'].unique():
        ...
        for FOLD in range(K_FOLDS):
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                # Validação (SND + AUC + CORAL Alignment)
                v_metrics = validate(model, val_loader, criterion, DATASET_NAME, device, unsup_loader=val_unsup_loader)
                val_loss, val_acc, val_sens, val_spec, val_f1, val_auc, val_thresh, val_snd, im_score, val_coral = v_metrics
                ...
                logging.info(f"Epoch {epoch:03d} | Tr_Loss: {tr_loss:.4f} | Tr_Loss_coral: {tr_loss_coral:.4f} | Val_SND: {val_snd:.4f} | Val_IM: {im_score:.4f} | Val_coral: {val_coral:.4f} | Val_AUC: {val_auc:.4f}| Test_AUC: {test_auc:.4f}")

                # Critério de Seleção Híbrido (Source AUC + Target SND - CORAL Div)
                # Penalizamos o Val_coral porque queremos que seja baixo (menos distância entre covariâncias)
                criterium_value = val_auc - 0.1 * val_coral
