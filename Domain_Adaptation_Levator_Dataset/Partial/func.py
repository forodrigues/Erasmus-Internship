...
def save_images(loader, save_folder="saved_images", max_images=None):
    ...
    print(f"Total of {count} images saved in {save_folder}")


def training_config(arc=ARCHITECTURE, device=DEVICE, optimizer=OPTIMIZER,weight_decay=WEIGHT_DECAY,pre_trained=PRE_TRAINED):
    global params

    model = BaseModel(backbone_name=arc,in_channels=1,num_classes=1,device=device,pretrained=True)    
...
def print_overlay (df,train_idx_final,val_idx_final,test_idx_final):
    ...
    train_ids = set(df.loc[train_idx_final, 'id'])
    val_ids = set(df.loc[val_idx_final, 'id'])
    test_ids = set(df.loc[test_idx_final, 'id'])
    ...
    if leak_train_val or leak_train_test or leak_val_test:
        logging.error("❌ CRITICAL: Data Leakage detetado!")
        if leak_train_val: logging.error(f"IDs em Treino e Validação: {leak_train_val}")
        if leak_train_test: logging.error(f"IDs em Treino e Teste: {leak_train_test}")
    else:
        logging.info("✅ Sucesso: Nenhum overlay de pacientes entre os conjuntos.")
