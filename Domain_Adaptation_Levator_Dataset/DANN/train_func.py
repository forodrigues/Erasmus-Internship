...
def validate(model, loader, criterion, DATASET, device, test=False, best_threshold=None, unsup_loader=None):
    ...
    with torch.no_grad():
        for batch_s in loader:
            ...
            # Lógica de concatenação para validação CDAN
            if unsup_iter:
    ...
    for DATASET in df['dataset'].unique():
        ...
        for FOLD in range(K_FOLDS): 
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                logging.info(f"Ep {epoch:02d} | GRL {lambda_grl:.2f} | Tr_Class_Loss { tr_cls_loss:.2f} | Tr_Dom_Loss { tr_dom_loss:.2f} |  Tr_Dom_Acc {dom_acc:.2f} | Tr_F1 {tr_f1:.2f} | Val_Loss {val_loss:.2f} | Val_Dom_Acc {val_dom_acc:.2f} | Val_Sens {val_sens:.4f} | Val_Spec {val_spec:.4f} |  Val_F1 {val_f1:.4f} | Val_AUC {val_auc:.4f} | Test_AUC {test_auc:.4f}")
                ...
                criterium = val_loss + 0.5 * domain_penalty
