...
def aspect_ratio_pres(img_tensor):
    """
    Mantém o aspecto da imagem adicionando padding central.
    Preserva metadados do MONAI MetaTensor.
    """
    # Assume que EnsureChannelFirstd já correu, logo o formato é [C, H, W]

    C, H, W = img_tensor.shape
    size = max(H, W)
    
    # Criar um novo tensor de padding preenchido com o valor mínimo
    # (importante em US para o fundo ser coerente com a imagem e não preto puro)
    if isinstance(img_tensor, torch.Tensor):
        # Cria o tensor no mesmo device (GPU) e com o mesmo tipo (float32)
        padded = torch.full(
            (C, size, size), 
            img_tensor.min(), 
        ...
        padded = np.full((C, size, size), img_tensor.min(), dtype=img_tensor.dtype)
    ...
    if hasattr(img_tensor, "copy_meta_from"):
        padded = img_tensor.copy_meta_from(padded)
...
def training_config(arc=ARCHITECTURE, device=DEVICE, optimizer=OPTIMIZER,weight_decay=WEIGHT_DECAY,pre_trained=PRE_TRAINED):
    ...
    model = DANNModel(backbone_name=arc,in_channels=1,num_classes=1,device=device,pretrained=True)    
...
def print_tensorboard(writer, FOLD, DATASET, lambda_grl, train_loss, train_loss_class, train_loss_dom, train_f1, val_loss, val_sens, val_spec, val_f1, val_auc, epoch):
    # Log do parâmetro de adaptação
    ...
    writer.add_scalar(f"Fold{FOLD}_{DATASET}/Train/Loss_Domain", train_loss_dom, epoch)    
