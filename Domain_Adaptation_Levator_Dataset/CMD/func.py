...
def training_config(arc=ARCHITECTURE, device=DEVICE, optimizer=OPTIMIZER,weight_decay=WEIGHT_DECAY,pre_trained=PRE_TRAINED):
    ...
    model = MMDModel(backbone_name=arc,in_channels=1,num_classes=1,device=device,pretrained=True)    
...
def print_tensorboard(writer,FOLD, DATASET, train_loss,train_loss_class,train_loss_mmd,train_f1, val_loss, val_sens,val_spec,val_f1,val_auc,epoch):
...
def create_split(df):
    ...
    for dataset_name in df['dataset'].unique():
        ...
        for fold_idx, (train_val_idx, test_idx) in enumerate(skf.split(x, y,groups)):
            ...
            gss = GroupShuffleSplit(test_size=0.25, random_state=42)  #tava 25 e 42
