...
def calculate_snd(features, temperature=0.05):
    ...
    return snd_value
# ==============================================================================
# FUNÇÃO DE LOSS (cmd - Correlation Alignment)
# ==============================================================================
def cmd_loss(source, target, k=5):
    """
    Calcula o CMD com suporte para decaimento de momentos.
    Assumes source e target já normalizados (ex: sigmoid -> [0,1]).
    """
    x1 = source.mean(0)
    x2 = target.mean(0)
    dl = torch.norm(x1 - x2, p=2) # L2 distance
    
    
    source_centered = source - x1
    target_centered = target - x2

    loss = dl
    
    for i in range(2, k + 1):
        m1 = torch.mean(torch.pow(source_centered, i), 0)
        m2 = torch.mean(torch.pow(target_centered, i), 0)
        
        moment_diff = torch.norm(m1 - m2, p=2)
        
        # Opcional: Adicionar um peso de decaimento se o loss explodir
        # weight = 1.0 / (i)  # Exemplo heurístico
        loss = loss + moment_diff
...
def train_one_epoch(model, sup_loader, unsup_loader, criterion, optimizer, scaler, DATASET, device, alpha):
    ...
    model.train()
    total_loss, total_loss_sup, total_loss_cmd = 0, 0, 0
    total_samples = 0
    ...
    for sup_batch in sup_loader:
        ...
        with torch.cuda.amp.autocast():
            ...
            if alpha > 0:
                ...
                # Para cmd, precisamos que o batch size do target > 1
                if imgs_unsup.size(0) > 1:
                    ...
                    features_sup = torch.sigmoid(features_sup)
                    features_unsup = torch.sigmoid(features_unsup)
                    loss_cmd = cmd_loss(features_sup, features_unsup, k=5)
                    loss = loss_sup + alpha * loss_cmd 
        ...
        total_loss_cmd += loss_cmd.item() * bs_sup 
    ...
    avg_loss_cmd = total_loss_cmd / total_samples if total_samples > 0 else 0.0

    return avg_loss, avg_loss_sup, avg_loss_cmd, acc, sens, spec, f1
...
def validate(model, loader, criterion, DATASET, device, test=False, best_threshold=None, unsup_loader=None):
    ...
    if unsup_loader is not None:
        ...
        if len(target_features_list) > 0 and len(source_features_list) > 0:
            ...
            # 3. CMD de Validação (Alinhamento Estatístico de Validação)
            # Nota: Usamos sigmoid se as features do teu modelo não forem limitadas (ex: ConvNeXt)
            val_cmd_alignment = cmd_loss(torch.sigmoid(all_features_source), torch.sigmoid(all_features_target), k=5).item()
    ...
    if test:
        ...
        return avg_loss, acc, sens, spec, f1, auc, best_threshold, snd_score, im_score, val_cmd_alignment
# ==============================================================================
# FUNÇÃO PRINCIPAL (OTIMIZADA PARA cmd)
# ==============================================================================
def train_kfold(dataset, DATASET_MANY, ARCHITECTURE, criterion, optimizer_cls, scheduler_cls,
    ...
    logging.info(f"Strategy: Pooled Cross-Validation (Domain Adaptation)")
    
    log_dir = f"../Tensorboard/cmd_unsup2" 
    ...
    for DATASET_NAME in df['dataset'].unique():
        ...
        for FOLD in range(K_FOLDS):
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                ALPHA_MAX = 0.25
                p = float(epoch - 1) / num_epochs
                gamma = 10
                ALPHA = ALPHA_MAX * (2.0 / (1.0 + np.exp(-gamma * p)) - 1)

     
                tr_metrics = train_one_epoch(model, train_loader, unsup_loader, criterion, optimizer, scaler, DATASET_NAME, device, ALPHA)
                tr_loss, tr_loss_sup, tr_loss_cmd, tr_acc, tr_sens, tr_spec, tr_f1 = tr_metrics

                # Validação (SND + AUC)
                v_metrics = validate(model, val_loader, criterion, DATASET_NAME, device, unsup_loader=val_unsup_loader)
                val_loss, val_acc, val_sens, val_spec, val_f1, val_auc, val_thresh, val_snd, im_score,val_cmd = v_metrics
                ...
                logging.info(f"Epoch {epoch:03d} | Alpha {ALPHA:.4f} |  Tr_Loss: {tr_loss:.4f} | Tr_Loss_CMD: {tr_loss_cmd:.4f} | Tr_F1: {tr_f1:.4f} | Val_SND: {val_snd:.4f} | Val_IM: {im_score:.4f} | Val_CMD: {val_cmd:.4f} | Val_AUC: {val_auc:.4f}| Test_AUC: {test_auc:.4f}")

                criterium_value =  val_auc - 0.1*val_cmd
